...
A standardised data cleaning pipeline is applied to each dataset to ensure consistency and reliability prior to RV construction. For all instruments, observations are validated against the applicable session boundaries for each trading day, with records falling outside the active window removed. Duplicate timestamps and observations with missing close prices are dropped to prevent undefined returns from entering the RV calculation. All timestamps in the raw data are recorded in Irish Standard Time (IST) or Irish Summer Time (GMT+1), reflecting the timezone of the Bloomberg terminal used for extraction, and the cleaning pipeline accounts for DST mismatch windows between Ireland and the United States when determining session boundaries.

For the SPX, U.S. market holidays and early-close sessions are excluded entirely, as partial trading days would produce RV estimates that are not directly comparable to those from full sessions. After cleaning, the SPX dataset contains 164 complete trading days from 22 July 2025 to 20 March 2026, with each retained day containing the expected 79 five-minute observations. For GBP/USD and XAU/USD, non-trading periods including weekends and exchange holidays are similarly removed, and the daily one-hour settlement break for XAU/USD is accounted for when defining session boundaries. A per-day bar count verification is performed across all datasets to confirm that each retained trading day contains the expected number of intraday observations.
...
The primary SPX sample spans 164 trading days from 22 July 2025 to 20 March 2026. The GBP/USD dataset covers 29 October 2025 to 26 March 2026, and the XAU/USD dataset spans 4 December 2025 to 26 March 2026. The historical SPX comparison dataset covers 1 May 2023 to 31 October 2023, yielding approximately 128 trading days after the removal of holidays and partial sessions. The variation in start dates across datasets reflects data availability from the Bloomberg Terminal at the time of extraction; the differing timeframes over which the models operate provide an opportunity to evaluate forecast stability across samples of varying length and market conditions.

For the SPX, the effective analysis window begins on 19 August 2025, after the 22-day rolling window required for the monthly RV component has been populated. The resulting analysis window of approximately seven months captures short-term volatility dynamics. The weekly and monthly RV components should therefore be interpreted as reflecting short-to-medium-horizon persistence rather than long-term structural behaviour, and the findings should be understood in the context of a relatively condensed sample that includes an atypical period of trade policy uncertainty.
...
\noindent where $s \geq 1$ is the smallest lag such that day $t-s$ is neither a macroeconomic announcement day nor a tariff event day. Within the estimation window, this adjustment modifies the daily lag on approximately 21 out of 140 trading days. The weekly and monthly components are not adjusted, as their rolling-average construction naturally dilutes the influence of any single announcement day.
...
In addition to the OLS-based HAR-RV specifications, a Random Forest regressor is estimated as a non-linear benchmark. The Random Forest is an ensemble method that constructs a large number of decision trees, each trained on a bootstrap sample of the data, and aggregates their predictions by averaging \citep{Breiman2001}. This approach is well-suited to the present setting for several reasons. First, the Random Forest can capture non-linear interactions between macroeconomic dummies and the autoregressive volatility components without requiring explicit specification of interaction terms. Second, the ensemble averaging reduces overfitting relative to a single decision tree, which is particularly important given the relatively short estimation windows available in this study. Third, \citet{Christensen2023} demonstrate that tree-based methods applied to RV forecasting can exploit regime-dependent patterns and adapt more rapidly to recent market movements than linear models, an advantage that may be especially relevant during the episode of elevated policy uncertainty captured by the primary sample. The Random Forest uses the same feature set as the corresponding HAR-RV specifications (the three RV components and the relevant macro and tariff dummies) and is trained on the same in-sample partition. Full hyperparameter details are reported alongside the results in Chapter~4.

\section{Evaluation Framework}
This section describes the forecast evaluation framework, including the train-test partitioning strategy, the performance metrics used to compare model specifications, and the cross-asset replication procedure.

\subsection{Train-Test Split Strategy}
Each dataset is partitioned into an in-sample estimation period and an out-of-sample evaluation period. For the primary SPX dataset, the in-sample period runs from 22 July 2025 to 29 January 2026 and the out-of-sample period from 30 January 2026 to 20 March 2026, yielding approximately 35 OOS trading days. For the historical SPX dataset, an 80:20 split is applied dynamically based on the number of complete trading days retained after cleaning, with the in-sample period covering approximately May to late September 2023 and the out-of-sample period spanning late September to October 2023. For GBP/USD (29 October 2025 to 26 March 2026) and XAU/USD (4 December 2025 to 26 March 2026), the same proportional split is applied to each dataset independently. Models are estimated on the in-sample data only, and forecasts are generated over the out-of-sample period by applying the estimated coefficients to lagged RV values that update as the evaluation window advances.

\subsection{Performance Metrics}
Model performance is evaluated using four complementary metrics, all computed on the log RV scale to match the estimation target.
...
The root mean squared error (RMSE) captures the average magnitude of forecast errors, defined as $RMSE = \sqrt{(1/n) \sum_{t=1}^{n} (y_t - \hat{y}_t)^2}$. RMSE ranges from 0 to $\infty$, with lower values indicating greater forecast accuracy. Because errors are squared before averaging, RMSE penalises large deviations more heavily than small ones, making it particularly sensitive to occasional large forecast failures \citep{Patton2015}.

The mean absolute error (MAE) provides an alternative measure of average forecast accuracy, defined as $MAE = (1/n) \sum_{t=1}^{n} |y_t - \hat{y}_t|$. MAE also ranges from 0 to $\infty$, with lower values preferred, but treats all errors equally regardless of magnitude, making it more robust to outliers than RMSE. Comparing RMSE and MAE jointly reveals whether a model's forecast performance is driven by consistent accuracy across all days (low MAE) or is disproportionately affected by a small number of large errors (high RMSE relative to MAE).
...
The full modelling and evaluation pipeline described above is applied independently to each of four datasets: the primary SPX sample (July 2025--March 2026), the historical SPX sample (May--October 2023), GBP/USD (October 2025--March 2026), and XAU/USD (December 2025--March 2026). While the core methodology is identical across assets, several differences warrant note.

First, because GBP/USD trades continuously for 24 hours per day and XAU/USD trades for 23 hours per day (with a one-hour daily settlement break), whereas the SPX session spans only 6.5 hours (09:30--16:00 ET), the number of five-minute return intervals per trading day differs across instruments. As daily RV is computed as the sum of squared intraday returns (Equation~3.2), longer trading sessions mechanically produce higher daily RV values, all else equal. This difference in scale does not affect the validity of cross-asset model comparisons, since all models are estimated on log RV and the relative performance of competing specifications within each asset is the primary object of interest. However, it implies that the absolute levels of RV and the magnitudes of estimated coefficients are not directly comparable across assets.

Second, the set of relevant macroeconomic events varies by asset: the Bank of England interest rate decision is included for GBP/USD but not for the other instruments, reflecting its direct relevance to sterling-denominated markets. The historical SPX dataset does not include a tariff dummy, as no material tariff events occurred during the May--October 2023 window. Results are compared across assets in Chapter~4 to assess whether the volatility effects of macroeconomic announcements are consistent or asset-specific.

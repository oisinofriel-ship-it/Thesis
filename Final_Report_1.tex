...
H$_{3}$$_{0}$ (Null): The RV response does not differ systematically across announcement innovation categories; the coefficients δ$_{0}$, δ$_{1}$, and δ$_{2}$ associated with anticipated, marginal, and significant announcement innovations are not monotonically ordered.

H$_{3}$$_{1}$ (Alternative): The RV response increases monotonically with the magnitude of the announcement innovation (δ$_{0}$ \textless{} δ$_{1}$ \textless{} δ$_{2}$), consistent with the notion that unanticipated information generates larger volatility responses than fully anticipated releases (Balduzzi et al., 2001; Bernanke and Kuttner, 2005).
...
H$_{4}$$_{1}$ (Alternative): Tariff announcements produce a positive and statistically significant increment in realised variance (λ \textgreater{} 0), reflecting the role of unscheduled policy uncertainty shocks as a distinct driver of short-term volatility (Baker, Bloom, and Davis, 2016).
...
FOMC interest rate decisions are sourced directly from the Board of Governors of the Federal Reserve System. Rather than using a survey-based consensus forecast, the expected rate outcome is derived from 30-Day Federal Funds futures prices traded on the Chicago Mercantile Exchange (CME). These futures contracts reflect what traders are collectively pricing the federal funds rate to be at expiration, providing a real-time, market-implied expectation of the FOMC decision in the period leading up to each announcement. This approach is consistent with \cite{@article{bauer-swanson-nberma,
...
}, who demonstrate that Federal Funds futures remain reliable and efficient measures of market rate expectations around scheduled FOMC announcements.
...
Eight macroeconomic event types are included in the analysis: FOMC interest rate decisions, Consumer Price Index (CPI) releases, Non-Farm Payroll (NFP) reports, Producer Price Index (PPI) releases, Personal Consumption Expenditure (PCE) price index releases, ADP National Employment Reports, Job Openings and Labor Turnover Survey (JOLTS) releases, and Initial Jobless Claims (CLAIMS). These events are selected on the basis that they are among the most closely monitored U.S. economic indicators and have been shown to generate significant volatility responses in prior empirical work (Andersen, Bollerslev, Diebold, and Vega, 2003; Gilbert, Scotti, Strasser, and Vega, 2017).

For each event, a binary dummy variable is set equal to one on days when the corresponding announcement occurs and zero otherwise. In addition, the expected-versus-surprise decomposition uses the actual release value, the consensus forecast, and the computed surprise (defined as the difference between the actual and forecast values). For rate-based indicators (FOMC, CPI, PPI, PCE), the surprise is expressed in percentage points; for count-based indicators (NFP, ADP, JOLTS, CLAIMS), the surprise is expressed in thousands. A release is classified as "expected" if the surprise is zero within a floating-point tolerance of 1 x 10\textasciicircum{}-9 and as a "surprise" otherwise.

\subsection{Tariff Announcement Data}
The sample period coincides with a period of heightened U.S. trade policy activity, during which a series of tariff announcements generated substantial market volatility. To capture these effects, a tariff announcement dummy is constructed using a manually curated event log (tariff_wss_v2.xlsx) that records the dates, descriptions, and severity assessments of tariff-related announcements during the period from August 2025 to February 2026.

Each tariff event is assigned a Weighted Severity Score (WSS) based on a qualitative assessment of its expected market impact, considering factors such as the scope of the tariff action, the size of affected trade flows, and the degree to which the announcement was anticipated. The binary tariff dummy (TARIFF) is set equal to one on days when at least one tariff event with a WSS of five or greater occurs, and zero otherwise. Events that were subsequently retracted or reversed are excluded from the dummy construction. Tariff events are further classified as escalatory or de-escalatory based on whether the announcement represented an increase or a reduction in trade barriers, enabling separate analysis of directional tariff effects on volatility.
...
Let P_{t,i} denote the closing price of the asset at the i-th five-minute interval on trading day t, where i = 1, 2, ..., M and M = 78 denotes the number of five-minute return intervals in a standard session. The intraday log-return at interval i is defined as:

\begin{equation}
r_{t,i} = ln(P_{t,i}) - ln(P_{t,i-1})        (3.1)
\end{equation}
Daily RV on trading day t is then computed as the sum of squared intraday log-returns:

\begin{equation}
RV_t = SUM_{i=1}\textasciicircum{}{M} r_{t,i}\textasciicircum{}2        (3.2)
\end{equation}
...
\begin{equation}
RV_t\textasciicircum{}(d) = y_{t-1}        (3.4)
\end{equation}
...
\begin{equation}
RV_t\textasciicircum{}(w) = (1/5) SUM_{j=1}\textasciicircum{}{5} y_{t-j}        (3.5)
\end{equation}
...
\begin{equation}
RV_t\textasciicircum{}(m) = (1/22) SUM_{j=1}\textasciicircum{}{22} y_{t-j}        (3.6)
\end{equation}
...
\begin{equation}
y_t = alpha + B_d * RV_t\textasciicircum{}(d) + B_w * RV_t\textasciicircum{}(w) + B_m * RV_t\textasciicircum{}(m) + e_t        (3.7)
\end{equation}
where y_t = ln(RV_t) is the dependent variable, and the regressors RV_t\textasciicircum{}(d), RV_t\textasciicircum{}(w), and RV_t\textasciicircum{}(m) are defined as in Equations (3.4) through (3.6). The coefficients B_d, B_w, and B_m capture the contribution of daily, weekly, and monthly volatility persistence, respectively. This specification serves as the benchmark against which all augmented models are compared.
...
\begin{equation}
y_t = alpha + B_d * RV_t\textasciicircum{}(d) + B_w * RV_t\textasciicircum{}(w) + B_m * RV_t\textasciicircum{}(m) + gamma * FOMC_t + e_t        (3.8)
\end{equation}
...
\begin{equation}
y_t = alpha + B_d * RV_t\textasciicircum{}(d) + B_w * RV_t\textasciicircum{}(w) + B_m * RV_t\textasciicircum{}(m) + gamma_1 * FOMC_t + gamma_2 * CPI_t + gamma_3 * NFP_t + e_t        (3.9)
\end{equation}
...
\begin{equation}
y_t = alpha + B_d * RV_t\textasciicircum{}(d) + B_w * RV_t\textasciicircum{}(w) + B_m * RV_t\textasciicircum{}(m) + delta_0 * MACRO_0_t + delta_1 * MACRO_1_t + delta_2 * MACRO_2_t + phi_1 * FOMC_exp_t + phi_2 * FOMC_unexp_t + e_t        (3.10)
\end{equation}
If volatility responds more strongly to larger surprises, the estimated coefficients should increase monotonically across categories (delta_0 \textless{} delta_1 \textless{} delta_2).
...
\begin{equation}
y_t = alpha + B_d * RV_t\textasciicircum{}(d) + B_w * RV_t\textasciicircum{}(w) + B_m * RV_t\textasciicircum{}(m) + SUM_{e,k} gamma_{e,k} * D_{e,k,t} + lambda * TARIFF_t + e_t        (3.11)
\end{equation}
...
\begin{equation}
The final HAR-RV extension addresses a potential contamination effect in the daily RV lag. On days immediately following a macroeconomic announcement or tariff event, the standard daily lag RV_t\textasciicircum{}(d) = y_{t-1} carries information about announcement-day volatility. If this elevated volatility is transient and does not reflect the underlying volatility regime, including it as a regressor may distort the model's forecasts for subsequent non-announcement days.
\end{equation}
...
\begin{equation}
RV_t\textasciicircum{}(d,adj) = y_{t-s}        (3.12)
\end{equation}
